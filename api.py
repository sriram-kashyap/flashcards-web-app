...
class UserAPI(Resource):
    def post(self):
        ...
        new_user = User(username=args['username'], password = generate_password_hash(args['password'], method='sha256'), email=args['email'])
        try:
...
class DeckAPI(Resource):
    ...
    def get(self, username):
        ...
        r=[]
        for deck in decks:
            r.append({'deck_name':deck.deck_name, 'score':deck.score, 'last_rev':(deck.last_rev.strftime("%d %b %Y, %I:%M %p"))})
        print(r)   
        return r
